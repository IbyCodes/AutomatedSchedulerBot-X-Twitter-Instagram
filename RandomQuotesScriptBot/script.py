...
# Working with PicSum + QuoteGarden
...
def post_scheduled_quote():
    ...
    if quote_text:
        ...
        # I've added this code to the main.py file instead as there will be times where the instagram account refuses to post another image until both the 'config' folder and 
...
post_scheduled_quote() # For now, I'll set it so that it just posts as soon as its run. 



# If you want to schedule the post to be uploaded to both Instagram and X/Twitter every day at ____ PM (System is a 24 H Clock)
#schedule.every().day.at("17:42").do(post_scheduled_quote)

# Keeping the script running (If you wanted to post every day, it will require the script to run at all times. However, make sure to deal with instagram config files as required! Instabot is very buggy with this, which is why I've provided the main.py!) 
#while True:
